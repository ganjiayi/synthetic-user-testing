...
// ─── Load files ───────────────────────────────────────────────────────────────
function loadPlan(runFolder) {
  const planPath = path.join(runFolder, 'plan.json');
  if (!fs.existsSync(planPath)) {
    console.error(`plan.json not found in ${runFolder}`);
    console.error('Run generatePlan.js first.');
    process.exit(1);
  }
...
function loadPersonaPrompts() {
  if (!fs.existsSync(PERSONA_PROMPT_PATH)) {
    console.warn(`personas/v4_system_prompts.md not found — using generic persona context`);
    return null;
  }
  return fs.readFileSync(PERSONA_PROMPT_PATH, 'utf8');
}
...
function parseTurnResponse(rawText, turnNumber, taskId, personaId) {
  ...
  try {
    ...
    console.warn(`   ⚠️  Turn ${turnNumber} parse failed — logging raw`);
    return {
      turn_number:  turnNumber,
      task_id:      taskId,
      persona_id:   personaId,
...
async function runPersonaSession(provider, persona, tasks, plan, personaLibrary, simulationPrompt) {
  ...
  for (const task of tasks) {
    console.log(`      📝 Task ${task.task_id}: ${task.task_name}`);
    let turnNumber = 1;
    let taskDone   = false;
      ...
      try {
        ...
        if (turn.screen_or_step === lastScreen) {
          ...
          if (consecutiveSameScreen >= stuckThreshold) {
            ...
            console.log(`      ⚠️  Stuck loop at turn ${turnNumber} — ${turn.screen_or_step}`);
          }
        } else {
        ...
        process.stdout.write(`         T${turnNumber} friction:${turn.eval_scores.friction_score ?? '?'} status:${completion}\n`);

        if (completion === 'completed') {
          tasksCompleted.push(task.task_id);
          taskDone = true;
          console.log(`         ✓ Completed at turn ${turnNumber}`);
        } else if (completion === 'abandoned') {
          taskDone = true;
          console.log(`         ✗ Abandoned at turn ${turnNumber} — ${turn.eval_scores.abandon_trigger || 'no reason given'}`);
        }
    ...
    if (!taskDone) {
      console.log(`         ⏱  Max turns reached for ${task.task_id}`);
    }
  }
...
function buildEvalMatrix(sessions) {
  ...
  for (const row of rows) {
    ...
    const s = summary[key];
    s.turn_count++;
    if (typeof row.friction_score === 'number') {
      s.friction_sum += row.friction_score;
      s.friction_count++;
    }
  ...
  for (const s of Object.values(summary)) {
    s.avg_friction = s.friction_count > 0
      ? Math.round((s.friction_sum / s.friction_count) * 10) / 10
      : null;
    delete s.friction_sum;
    delete s.friction_count;
  }
...
// ─── Write outputs ────────────────────────────────────────────────────────────
function writeOutputs(runFolder, sessionLog, evalMatrix) {
  const sessionsDir = path.join(runFolder, 'sessions');
  fs.mkdirSync(sessionsDir, { recursive: true });

  const logPath    = path.join(sessionsDir, 'session_log.json');
  const matrixPath = path.join(sessionsDir, 'eval_matrix.json');

  fs.writeFileSync(logPath,    JSON.stringify(sessionLog, null, 2));
  fs.writeFileSync(matrixPath, JSON.stringify(evalMatrix, null, 2));

  return { logPath, matrixPath };
}
...
async function main() {
  ...
  const allEvalKeys    = plan.eval_metrics?.all_keys || [];

  console.log(`\n🧪 Starting evaluation — ${plan._meta?.run_id || 'unknown run'}`);
  console.log(`   Product:   ${plan.research_goals?.product_name || '(unknown)'}`);
  ...
  const sessionLog = {
    run_metadata: {
      ...
      tasks_run:          activeTasks.map(t => t.task_id),
      eval_keys_used:     allEvalKeys,
      artefact_config:    plan.study_context?.artefact_config || {},
    },
    sessions,
    eval_matrix_ref: 'eval_matrix.json',
  };

  const evalMatrix = {
    run_id:       sessionLog.run_metadata.run_id,
    generated_at: new Date().toISOString(),
    ...buildEvalMatrix(sessions),
  };

  const { logPath, matrixPath } = writeOutputs(absRunFolder, sessionLog, evalMatrix);

  appendRunLog(absRunFolder, `evaluator complete — session_log and eval_matrix written`);

  console.log('\n✅ Evaluation complete');
  console.log('\n📊 Session summary:');
  for (const s of sessions) {
    const scores = s.turns.map(t => t.eval_scores?.friction_score).filter(f => typeof f === 'number');
    const avg    = scores.length > 0
      ? (scores.reduce((a, b) => a + b, 0) / scores.length).toFixed(1)
      : 'n/a';
    console.log(`   ${s.persona_name.padEnd(30)} outcome: ${s.session_outcome.padEnd(25)} avg friction: ${avg}  stuck loops: ${s.stuck_loop_flags.length}`);
  }

  console.log(`\n📁 session_log.json  → ${logPath}`);
  console.log(`📁 eval_matrix.json  → ${matrixPath}`);
  console.log('\nNext step: python3 src/pipeline/reporter.py ' + runFolder);
}
