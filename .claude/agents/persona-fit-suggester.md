...
1. **Read confirmed context.** `q3_goals.primary_rq`, `q3_goals.secondary_rqs`, `q5_product_context` (product/feature under test), and `q2_context` if it affects who'd plausibly be using this artefact (e.g. a live-product test vs. an early wireframe may change which personas' tech-literacy tier is even relevant).
